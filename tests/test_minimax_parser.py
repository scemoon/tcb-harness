...
class TestMinimaxToolCallParser:
    def test_single_invoke(self):
        from onecode.agent.engine import _extract_minimax_tool_uses
        text = (
...
class TestInstanceToolIdCounter:
    ...
    def test_counter_persists_across_legacy_extractions(self):
        from onecode.agent.engine import AgentEngine, _extract_legacy_tool_uses
    ...
    def test_minimax_then_legacy_no_collision(self):
        from onecode.agent.engine import (
...
class TestProviderErrorClassification:
    def test_429_is_rate_limit(self):
        from onecode.models.errors import RateLimitError
        from onecode.models.provider import Provider
    ...
    def test_401_is_auth_error(self):
        from onecode.models.errors import AuthError
        from onecode.models.provider import Provider
        err = Provider.classify_http_error(401, "invalid api key")
        assert isinstance(err, AuthError)

    def test_5xx_is_transient(self):
        from onecode.models.errors import TransientProviderError
        from onecode.models.provider import Provider
    ...
    def test_400_with_context_length(self):
        from onecode.models.errors import ContextLengthError
        from onecode.models.provider import Provider
    ...
    def test_400_without_context_length_is_generic(self):
        from onecode.models.errors import ProviderError
        from onecode.models.provider import Provider
...
class TestDailyLogRotation:
    ...
    def test_log_writes_appear_in_file(self, tmp_path: Path, monkeypatch):
        ...
        root = setup_logging("INFO")
        try:
            logging.getLogger("onecode.agent.engine").info("engine log line")
    ...
    def test_default_level_is_info_not_warning(self, tmp_path: Path, monkeypatch):
        ...
        try:
            ...
            # The onecode namespace should now also be at INFO, not WARNING.
            assert logging.getLogger("onecode").level <= logging.INFO
            assert logging.getLogger("onecode.agent.engine").level <= logging.INFO
        finally:
            for h in list(root.handlers):
                root.removeHandler(h)
                h.close()
...
class TestSandboxResourceLimits:
    ...
    def test_sandbox_has_public_exec_method(self):
        ...
        from onecode.agent.tools.sandbox import Sandbox, SandboxConfig, SandboxMode
        from pathlib import Path
    ...
    def test_shell_exec_does_not_apply_rlimit_to_parent(self, tmp_path):
        ...
        If the parent process is given a 512 MB RLIMIT_AS, the onecode import
