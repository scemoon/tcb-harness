...
## WEB-E2E-001: E2E Test Required
**Severity:** MUST
**Description:** If a WEB component exists, `apps/web/tests/e2e/` MUST contain `.spec.ts` files.
**Valid:** `ls apps/web/tests/e2e/*.spec.ts` returns test files.
**Invalid:** No `.spec.ts` files found when WEB component exists.

## WEB-E2E-002: E2E Tests Must Pass
**Severity:** MUST
**Description:** All Web E2E tests MUST pass before marking verify phase complete.
**Valid:** `npx playwright test --project=chromium` exits with code 0.
**Invalid:** Any Playwright test fails.

## WEB-E2E-003: Base URL Must Be Reachable
**Severity:** MUST
**Description:** The target application base URL MUST be accessible before running E2E tests.
**Valid:** `curl -f http://localhost:{PORT}` returns HTTP 200.
**Invalid:** Cannot connect to the application URL.
